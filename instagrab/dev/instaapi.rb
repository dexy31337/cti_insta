...
hashtags << 'мото' << 'мотокросс' << 'эндуро'
hashtags << 'сноуборд'<<'горныелыжи'
hashtags << 'спорт' << 'экстрим'
hashtags << 'снежком' << 'snejcom'
hashtags << 'красная_поляна' << 'krasnaya_polyana'
hashtags << 'кайт' << 'серфинг'
#hashtags = ['ctibrace']
...
conn = PGconn.new('192.168.0.52','5432','','','instadb','insta','qdvvhZ37a')
